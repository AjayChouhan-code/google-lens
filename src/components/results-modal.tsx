...
import { X, Crop, Loader2, Search, Type, Languages, Upload, Menu } from 'lucide-react'
...
export function ResultsModal({ isOpen, onClose, imageFile }: ResultsModalProps) {
      ...
      <div className="flex flex-1">
        ...
        <div className="w-1/2 bg-[#202124] p-6 flex flex-col">
            ...
            {imageUrl && (
              <div className="relative w-full h-full">
                ...
                <div className="absolute inset-0">
                  ...
                  >
                    {/* Corner handles */}
                    <div className="absolute -left-1 -top-1 w-2 h-2 bg-white cursor-nw-resize" onMouseDown={(e) => handleMouseDown(e, 'nw')} />
                    <div className="absolute -right-1 -top-1 w-2 h-2 bg-white cursor-ne-resize" onMouseDown={(e) => handleMouseDown(e, 'ne')} />
                    <div className="absolute -left-1 -bottom-1 w-2 h-2 bg-white cursor-sw-resize" onMouseDown={(e) => handleMouseDown(e, 'sw')} />
                    <div className="absolute -right-1 -bottom-1 w-2 h-2 bg-white cursor-se-resize" onMouseDown={(e) => handleMouseDown(e, 'se')} />
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="mt-6 flex gap-0 justify-center w-max m-auto rounded-3xl bg-[#303134]">
            <Button variant="secondary" className="bg-white text-gray-900 hover:bg-gray-100 rounded-3xl">
              Search
            </Button>
            <Button variant="ghost" className="bg-[transparent] text-gray-300 hover:bg-[#404144] border-none rounded-none	">
              Text
            </Button>
            <Button variant="ghost" className="bg-[transparent] text-gray-300 hover:bg-[#404144] rounded-none	">
              Translate
            </Button>
          </div>
        </div>
        ...
        <div className="w-1/2 bg-white p-6 relative">
          <div className="flex-1 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 144px)' }}>
            <h2 className="text-lg font-semibold text-gray-900 mb-6">Visual matches</h2>
            {isProcessing ? (
              ...
              <div className="grid grid-cols-2 gap-6">
                {results.map((result) => (
                  <div key={result.id} className="bg-gray-50 p-6 rounded-lg flex flex-col items-center">
                    <CircularProgress
                      percentage={result.percentage}
                      color={result.color}
                    />
                    <p className="mt-4 text-sm font-medium text-gray-900">{result.title}</p>
                  </div>
                ))}
              </div>
            )}
